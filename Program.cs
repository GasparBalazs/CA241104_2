...
Console.WriteLine($"elit versenyzok: {f1} fo");

var f2 = versenyzok
    .Where(v => !v.Nem)
    .Average(v => 2014 - v.Szul);
Console.WriteLine($"nok atlageletkora: {f2:0.00} ev");

var f3 = versenyzok
    .Sum(v => v.VersenyIdok["kerékpározás"].TotalHours);
Console.WriteLine($"kerékpározás toltott ido: {f3:0.00} ora");

var f4 = versenyzok
    .Where(v => v.Kategoria == "elit junior")
    .Average(v => v.VersenyIdok["úszás"].TotalMinutes);
Console.WriteLine($"elit junior k.ban az atlagos uszassal toltott ido: {f4:0.00} perc");
